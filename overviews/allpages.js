...
if (user_data.overviews.addFancyImagesToOverviewLinks) {
	...
	overviewLinks.each(function(index) {
		...
		switch (index) {
			case 0:
//				overviewLink.parent()
//                    .css("background-image", 'url("https://cdn.tribalwars.net/graphic/icons/header.png")')
...
//				overviewLink.prepend("&nbsp; &nbsp;");
			...
			case 3:
				if (overviewLink.parent().hasClass("selected")) {
					$("table.modemenu:last a", content_value).each(function(index) {
						...
						if (imageToAdd !== "") {
							$(this).prepend("<img src='https://cdn.tribalwars.net/"+imageToAdd+"' title='"+overviewLink.text() + " &gt; " + $(this).text()+"' /> &nbsp;");
						}
					});
				}
			...
			case 8:
				...
				overviewLink.prepend("<img src='https://cdn.tribalwars.net/"+imageToAdd+"' title='"+overviewLink.text()+"' /> &nbsp;");
